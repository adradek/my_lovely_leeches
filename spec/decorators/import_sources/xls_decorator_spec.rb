...
RSpec.describe ImportSources::XlsDecorator do
  subject(:decorator) { described_class.new(import_source) }

  let(:import_source) do
    create(:import_source, format: :xls).tap do |source|
      source.source.attach(fixture_file_better_upload("sample.xls"))
    end
  end

  describe ".new" do
    context "when import_source is xls with an attached file" do
      it "initializes without raising" do
        expect { decorator }.not_to raise_error
      end
    end

    context "when import_source is not xls format" do
      let(:import_source) { build_stubbed(:import_source, format: :xlsx) }

      it "raises an invalid format error" do
        expect { decorator }.to raise_error(/Invalid import source format: xlsx/)
      end
    end

    context "when import_source has no attached file" do
      let(:import_source) { create(:import_source, format: :xls) }
  ...
  describe "#sheets" do
    ...
    let(:expected_names) { ["2025 г.", "Графики, диаграммы", "Среднее", "Биоиндикация"] }
